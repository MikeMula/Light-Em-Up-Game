...
class Game {
    ...
    public function __construct(Lights $lights, $dir, $file) {
        ...
        $this->userid = $lights->getUserId(); //will be null if user is not logged in
        $this->lights = $lights;
        $this->file = $file;
    ...
    /**
     * Fill grid of game with data from db
     * @param $file
     */
    public function loadGameFromDb($file) {

        $gamesTable = new GamesTable($this->lights->getSite(), $this->lights);
        $row = $gamesTable->getDbGame($file, $this->lights->getUserId());

        $gameid = $row['id'];
        $this->id = $gameid; //set the game id for this game object

        echo $this->id;
        echo "   ";

        $cells = new Cells($this->lights->getSite());
        //Get all the cells in the db with this game id
        $allgamecells = $cells->getCells($gameid); //returns an array of Cell objects
        foreach($allgamecells as $cell) {
            $r = $cell->getRow();
            $c = $cell->getCol();
            $val = $cell->getValue();
            $this->grid[$r][$c] = $val;
    ...
    /**
     * Save the move made by updating the cell value in the db
     * @param $gameid
     * @param $row
     * @param $col
     * @param $val
     */
    public function saveGame($gameid, $row, $col, $val) {
        $cells = new Cells($this->lights->getSite());
        $cells->updateCell($gameid, $row, $col, $val);
    }
    ...
    public function clearDb() {
        $gamesTable = new GamesTable($this->lights->getSite(), $this->lights);
        $gamesTable->clearDbGame($this->id);
    }
    ...
    public function click($r, $c) {
        ...
        //if a user is logged in, save the game
        switch($value) {
            case self::CELL_UNSET:
                $this->grid[$r][$c] = self::LIGHT;
                if($this->lights->isLoggedIn()) { $this->saveGame($this->id, $r, $c, self::LIGHT); }
                break;

            case self::UNLIGHT:
                $this->grid[$r][$c] = self::CELL_UNSET;
                if($this->lights->isLoggedIn()) { $this->saveGame($this->id, $r, $c, self::CELL_UNSET); }
                break;

            case self::LIGHT:
                $this->grid[$r][$c] = self::UNLIGHT;
                if($this->lights->isLoggedIn()) { $this->saveGame($this->id, $r, $c, self::UNLIGHT); }
                break;
        }
    }
    ...
    private $lights;
    private $userid = null; //internal user ID of user playing this game
    private $id; //this games ID
}